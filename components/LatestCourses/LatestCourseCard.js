...
const LatestCourseCard = (props) => {
    return(
        <div className={['col-8', 'col-md-3', 'px-2'].join(' ')} style={{height: '100%'}}>
          <Link href={props.info.url}>
            <a className={['d-flex', 'pointer', 'flex-column', styles.banner].join(' ')} style={{height: '100%', border: '1px solid #dedede', backgroundColor: 'white', borderRadius: '4px'}}>
              ...
              <div className={['d-flex', 'flex-row', 'rtl', 'px-3', 'pb-3', 'mt-auto'].join(' ')}>
                {
                  props.info.off === 0
                  ?
                  <p className={['rtl', 'py-1', 'px-2', 'text-right', 'm-0', 'rounded'].join(' ')} style={{fontSize: '14px', backgroundColor: '#DEDEDE'}}>{props.info.price.toLocaleString() + ' تومان'}</p>
                  :
                  (
                    <React.Fragment>
                      <p className={['rtl', 'py-1', 'px-2', 'text-right', 'm-0', 'rounded'].join(' ')} style={{fontSize: '14px', backgroundColor: '#DEDEDE'}}><del>{props.info.price.toLocaleString()}</del></p>
