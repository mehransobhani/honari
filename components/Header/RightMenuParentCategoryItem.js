    ...
    return(
        <div className={['px-2', 'py-3'].join(' ')} style={{borderBottom: '1px solid #DEDEDE'}}>
            <div className={['d-flex', 'flex-row', 'rtl', 'align-items-center', 'justify-content-between'].join(' ')}>
                <Link href={information.parentUrl.substr(18)}><a className={['mb-0'].join(' ')} style={{fontSize: '15px', color: showChildrenCategories ? "#00BAC6" : "#2B2B2B"}}>{information.parentName}</a></Link>
                {
            ...
            </div>
            {
                ...
                ?
                    information.children.map((child, counter) => {
                        return <Link key={counter} href={child.url.substr(18)}><a><h6 className={['text-right', 'rtl', 'pr-3', 'mt-3', 'mb-0', 'pointer'].join(' ')} style={{fontSize: '13px'}}>{child.name}</h6></a></Link>
                    })
                :
                    null
            }
