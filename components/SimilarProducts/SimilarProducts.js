            ...
            <div className={['w-100', 'd-flex', 'flex-row', 'rtl', 'px-2', 'p-md-0', 'align-items-stretch', styles.newProductContainer].join(' ')} style={{overflowX: 'scroll'}}>
                {
                    products.map((product, counter) => {
                            return(
                                <SimilarProductCard imageUrl={'https://honari.com/image/resizeTest/shop/_600x/thumb_' + product.prodID + '.jpg'} name={product.name} price={product.price} discountedPrice={product.price} anchor={product.url}/>
                            );
                    })
                }
            </div>
